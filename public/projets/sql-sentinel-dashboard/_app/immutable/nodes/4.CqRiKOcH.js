...
trade_prices AS (
  SELECT
    ...
    m.market_price,
    -- Calcul précis du slippage relatif
    ABS(t.price - m.market_price) / NULLIF(m.market_price, 0) AS slippage_pct
  FROM transactions t
  -- Jointure précise à la minute pour éviter les décalages irréalistes
  INNER JOIN market_prices m
...
slippage_summary AS (
  ...
  FROM trade_prices
  -- On filtre les anomalies aberrantes (ex: > 10%) pour la propreté du graphique
  WHERE slippage_pct < 0.1 
  GROUP BY ALL
...
SELECT * FROM risk_summary`),t(13,F=wt`WITH transactions AS (
  SELECT * FROM sentinel.transactions
),
suspicious_pairs AS (
  SELECT
...
WHERE status = 'completed'
GROUP BY 1
ORDER BY 1`),[d,J,U,he,te,Se,n,r,O,k,E,M,W,F,V,j,$,I,g,P,Y,z,ee,ne,be,se,v,ce,B,le,de,p,i]}class yr extends Mt{constructor(e){super(),Rt(this,e,or,ar,Ot,{data:6},null,[-1,-1])}}export{yr as component};
